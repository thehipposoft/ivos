  ...
  return (
    <div ref={container} className={`${hiddeOnDesktop ? 'lg:hidden' : 'lg:block'} w-screen fixed z-50 top-0`} id='header-nav'>
        <div className='md:w-[85vw] w-screen flex justify-between mx-auto items-center relative'>
            <div className=' p-2 rounded-lg ml-4 md:ml-0'>
            ...
            <div className={`${toggleMenu ? 'md:translate-y-0 translate-y-24' : ''} rounded-bl-[15px] duration-700 z-30 md:z-0 absolute -translate-y-full md:h-[80px] h-96 md:w-[85vw] w-screen flex md:justify-start lg:justify-center justify-around mx-auto items-center bg-[#F94E19]`}>
              <nav className='flex md:flex-row md:flex-wrap flex-col justify-center items-center lg:gap-20 md:gap-10 gap-8 md:pl-6 lg:pl-0 pt-10 md:pt-0'>
                    <Link onClick={handleToggleMenu} className={`lg:hidden text-white duration-500 text-[18px] medium hover:text-[#01222C]`} href={'/'}>Home</Link>
